...
module.exports.Create = async (req, res) => {
    ...
    try {
        ...
        logger.withFields({"error": error}).debug(`Search profile: ${nickname}`);
        JSONResponse.Send(res, 500, null);
        return;
    }      
    ...
    try {
        ...
        logger.withFields({"error": error}).debug(`Search user: ${nickname}`);
        JSONResponse.Send(res, 500, null);
        return;
    }          
    ...
    if (!user) {
        try {
            ...
            logger.withFields({"error": error}).debug(`Unable to update profile: ${nickname}`);
        ...
        try {
            ...
            logger.withFields({"error": error}).debug(`Unable to create profile: ${nickname}`);
