<template>
    ...
    <form name="form" @submit.prevent="handleSubmit(!v$.$invalid)" class="w-full" v-if="isLogged">
      <div class="w-full">
        <h3>Title</h3>
        <InputText type="text" v-model="state.title" />
        ...
        <client-only placeholder="loading...">
          <h3>Preview</h3>
          <Editor editorStyle="height: 160px" v-model="state.preview"/>
        </client-only>
      </div>
  ...
  export default {
    ...
    setup(_, ctx) {
      ...
      const handleSubmit = (isFormValid) => {
        ...
        $addPost(state.post, state.tags, state.preview, state.title).then(() => {
          message.value = "";
